...
void jl_throw_in_thread(int tid, mach_port_t thread, jl_value_t *exception)
{
    ...
    *(jl_all_task_states[tid].pbt_size) = rec_backtrace_ctx(jl_all_task_states[tid].bt_data, MAX_BT_SIZE, (bt_context_t)&state);
    *(jl_all_task_states[tid].pexception_in_transit) = exception;
