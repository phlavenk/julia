...
jl_value_t *jl_toplevel_eval_flex(jl_value_t *e, int fast)
{
    ...
    if (ewc) {
        ...
        result = jl_call_method_internal(thk, NULL, 0);
    }
    else {
                                              ...
                                              jl_binding_t *bnd)
{
    ...
    if (*bp != NULL) {
        gf = *bp;
        if (!jl_is_datatype_singleton(jl_typeof(gf)))
...
DLLEXPORT jl_value_t *jl_method_def(jl_sym_t *name, jl_value_t **bp, jl_value_t *bp_owner,
                                    ...
                                    jl_value_t *isstaged, jl_value_t *call_func, int iskw)
{
    jl_module_t *module = (bnd ? bnd->owner : NULL);
    ...
    if (!(jl_is_type_type(ftype) || (jl_is_datatype(ftype) && !((jl_datatype_t*)fype)->abstract)))
