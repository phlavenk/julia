...
DLLEXPORT jl_lambda_info_t *jl_instantiate_staged(jl_methlist_t *m, jl_tupletype_t *tt, jl_svec_t *env)
{
    ...
    func = (jl_lambda_info_t*)jl_toplevel_eval_in(m->func->module, (jl_value_t*)ex, 1); // need to eval macros in the right module, but not give a warning for the `eval` call unless that results in a call to `eval`
...
static void precompile_unspecialized(jl_lambda_info_t *func, jl_tupletype_t *sig, jl_svec_t *tvars)
{
    assert(sig);
    func->specTypes = sig;
    jl_gc_wb(func, sig);
...
void jl_compile_all_defs(jl_function_t *gf)
{
    ...
    while (m != (void*)jl_nothing) {
        ...
        if (m->func->unspecialized == NULL) {
            func = jl_add_static_parameters(m->func, jl_emptysvec);
            m->func->unspecialized = func;
            jl_gc_wb(m->func, func);
    ...
    if (m->constant_table != NULL) {
        for(i=0; i < jl_array_len(m->constant_table); i++) {
            ...
            if (jl_is_lambda_info(el)) {
                ...
                jl_lambda_info_t *func = li->unspecialized;
                if (func == NULL) {
                    func = li;
...
jl_value_t *jl_apply_generic(jl_value_t **args, uint32_t nargs)
{
    jl_value_t *F = args[0];
...
{
    jl_datatype_t *ftype = jl_new_datatype(name, jl_any_type, jl_emptysvec,
                                           jl_emptysvec, jl_emptysvec, 0, 0, 0);
    ftype->name->module = module;
    ftype->name->mt->module = module;
    // TODO jb/functions: might need to mark jl_typeof(v) in gc now
    return jl_new_struct(ftype);
}
...
// -1 for no limit.
DLLEXPORT
jl_value_t *jl_matching_methods(jl_value_t *types, int lim)
{
    assert(jl_nparams(types) > 0);
    assert(jl_is_datatype(jl_tparam0(types)));
    jl_methtable_t *mt = ((jl_datatype_t*)jl_tparam0(types))->name->mt;
